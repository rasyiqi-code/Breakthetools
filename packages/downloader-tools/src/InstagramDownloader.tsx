...
import { parseInstagramUrl, type ParsedUrl } from './utils/urlParser'
...
export function InstagramDownloader() {
    ...
    useEffect(() => {
        if (url.trim()) {
            ...
            setParsedUrl(parsed)
            if (!parsed.isValid) {
                setError('URL Instagram tidak valid')
            } else {
                setError('')
            }
        } else {
    ...
    const fetchMediaInfo = useCallback(async () => {
        ...
        try {
            const apiUrl = `/api/downloader/instagram?id=${parsedUrl.id || ''}&url=${encodeURIComponent(url)}`
            ...
            setError(err.message || 'Gagal mengambil informasi dari Instagram')
            setMediaInfo(null)
        } finally {
            setLoading(false)
        }
    ...
    const handleDownload = async () => {
        ...
        try {
            ...
            const baseName = mediaInfo.title || 'instagram-media'
    ...
    return (
        <div className="max-w-full sm:max-w-4xl mx-auto px-4">
            <div className="mb-4 sm:mb-6">
                <h1 className="text-2xl sm:text-3xl font-bold text-neutral-900 mb-2">Instagram Downloader</h1>
                <p className="text-sm sm:text-base text-neutral-600 mb-2">Download foto, video, reels, dan stories dari Instagram</p>
            </div>
                    ...
                    <div className="tool-card p-4 sm:p-6 mb-4 sm:mb-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                            {mediaInfo.thumbnail ? (
                                ...
                                <div className="relative rounded-lg overflow-hidden border border-neutral-200 bg-neutral-100 flex items-center justify-center aspect-video">
                                    <Video className="w-12 h-12 sm:w-16 sm:h-16 text-neutral-400" />
                                </div>
                            )}
